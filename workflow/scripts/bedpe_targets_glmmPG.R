...
if (interactive()) { ## fake data for testing
  ...
  snakemake <- Snakemake(
    input=list(bedpe_seqlet="/work/aaa/projects/chrombpnet-devmult/pipeline/results/chrombpnet_nobias/pretrained_bias/IM-B/mean/modisco/IM-B_mean.counts_scores.top_tf.bedpe.gz",
               bedpe_glmmpg="/work/DevM_analysis/data/E2G/glmmPG/IM-B.glmmPG_FDR20.bedpe"),
    output=list(bedpe="/work/aaa/projects/chrombpnet-devmult/pipeline/results/seqlet-target/glmmPG/IM-B_seqlet_glmmPG.counts_scores.bedpe.gz",
                csv="/work/aaa/projects/chrombpnet-devmult/pipeline/results/seqlet-target/glmmPG/IM-B_seqlet_glmmPG.counts_scores.csv.gz"),
...
df_glmmpg <- read_tsv(snakemake@input$bedpe_glmmpg,
                     col_names=c("chrom1", "start1", "end1",
                                 "chrom2", "start2", "end2",
                                 "name", "score",
                                 "strand1", "strand2")) %>%
...
df_combined <- df_overlap %>%
  ...
  left_join(df_glmmpg, by="idx_enhancer", suffix = c("_seqlet", "_target")) %>%
  ...
  transmute(
    ...
    seqlet_score=score_seqlet, peak_score=target_score, glmmpg_score=score_target,
    modisco_id=modisco_id, pattern_name=pattern_name,
    tf_top=name_seqlet, glmmpg_id=name_target, target_name=target_name
  )
...
df_summary <- df_combined %>%
  ...
  arrange(target_name) %>%
  summarize(n_targets=length(unique(target_name)),
            target_names=str_c(unique(target_name), collapse=";"),
            target_ids=str_c(unique(target_name), collapse=";"),
            mean_glmmpg_score=mean(glmmpg_score),
            median_glmmpg_score=median(glmmpg_score),
